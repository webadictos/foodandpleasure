<?php
/**
 * Template part for displaying carrusel posts
 *
...
if (has_post_thumbnail()):
    ...
    if($i===0){
      ...
      $thumb ='<img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" data-lcp-src="'.$thumbSrc[0].'" data-lcp-loaded="false" alt="'.get_the_title().'" title="'.get_the_title().'" class="d-block w-100" loading="lazy">';
    }else{
        $thumb = get_the_post_thumbnail(get_the_ID(), 'full', array('title' => get_the_title(), 'alt' => get_the_title(), 'class' => "d-block w-100"));
    } 


   // $thumb = get_the_post_thumbnail(get_the_ID(),'large',array( 'title' => wp_strip_all_tags(get_the_title()), 'alt'=> wp_strip_all_tags(get_the_title()),'class'=>'d-block w-100'));
    else:
